...
int parsePosInt(char *arg) {
  ...
  try {
    ...
    cout << "Invalid value for ksize (kernel window size)" << endl;
    exit(3);
  }
  return size;
}
...
Mat getImg(char *filename) {
  ...
  if (image.empty()) {
    ...
    cout << "Could not read origin image at " << filename << endl;
    exit(2);
  }
  return image;
}
...
void putImg(Mat image, char *filename) {
  ...
  // Write a image in a png file using compression in the path specified in
  // filename variable
  ...
  if (!imwrite(filename, image, compression_params)) {
    ...
    cout << "Could not write destination image at " << filename << endl;
    exit(4);
  }
}
