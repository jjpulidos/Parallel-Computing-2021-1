...
int main (int argc, char *argv[]) {
    ...
    // Converting BGR default color mode to HSV in order to apply Median Filter Correctly
    // Median Filter shouldnt be applied in RGB or BGR color modes because are not going to get the real Median between
    // all the channels, If you are using a GrayScale Image is ok, but with Color Images you should do a HSV conversion
    ...
    // Instead of using malloc/free we drawn on C++ Memory Management and functions like "clone" in OpenCV  library
    dst_h = new_h.clone();
    dst_s = new_s.clone();
    dst_v = new_v.clone();
    ...
    // After applied MedianFilter for each channel, were merged due to build the Image Again
